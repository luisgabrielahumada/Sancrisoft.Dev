...
namespace FinanControl.Api.Controllers
{
    ...
    {
        private readonly IExpenses _process;
        ...
        public HttpResponseMessage Post([FromBody]Expenses row)
        {
            var id = _process.Insert(row, UpdatedId);
            //var item = _process.Get(id);
            //Notifications.NotifyClientActivity(item, TypeNotifications.Notifications, $"+ Inv {item.Product.Name}, Cant: {item.Quantity},${item.PriceSale}", HttpStatusCode.OK);
            return Request.CreateResponse(HttpStatusCode.OK);
        }
        [HttpPost]
        [HttpPut]
        // PUT: api/Users/5
        public HttpResponseMessage Put(Guid id, [FromBody]Expenses row)
        {
